...
function UpdateTodo({isAuthenticated, setIsAuthenticated, match}) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://44.210.188.43:3001/api/todo/${match.params.id}`, {title, targetDate}, {
        headers: {
  ...
  useEffect(() => {
    const loadData = async () => {
      let response = null;
      try {
        response = await axios.get(`http://44.210.188.43:3001/api/todo/${match.params.id}`, {
          headers: {
						'Authorization': `Bearer ${sessionStorage.getItem('token')}`
					}
        });
